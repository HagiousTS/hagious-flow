...
const HEALTH_CHIP: Record<string, { label: string; cls: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  off_track: { label: 'Fora do prazo', cls: 'tag-priority-high', icon: AlertTriangle },
}
...
function calcMargin(project: Project, hoursLogged: number): number | null {
  ...
  const projectedActualHours = Math.max(hoursLogged, Number(project.actual_hours ?? 0))
  const hourlyRate = Number(project.cost_estimate ?? 0) / Number(project.estimated_hours)
  if (!Number.isFinite(hourlyRate) || hourlyRate === 0) return null
  const cost = projectedActualHours * hourlyRate
  const margin = ((Number(project.budget_amount) - cost) / Number(project.budget_amount)) * 100
  return margin
}

export function ProjectHeader({ project, health, totalHoursLogged }: ProjectHeaderProps) {
  ...
  const hoursLogged = Math.max(totalHoursLogged, Number(project.actual_hours ?? 0))
  ...
  const isLate =
    project.due_date && new Date(project.due_date).getTime() < Date.now() && progress < 100
    ...
    <div className="flex items-start gap-5 flex-wrap">
      ...
      <div className="flex-1 min-w-[280px]">
        ...
        <p className="text-sm text-muted mt-1">
          ...
          {project.sponsor_name && (
            <>
              {' · Sponsor: '}
              <span className="text-text font-medium">{project.sponsor_name}</span>
            </>
          )}
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Mini
          ...
          sub={dueRelative}
          accent={isLate ? 'text-danger' : project.due_date ? 'text-warn' : 'text-text'}
        />
        <Mini
          label="Progresso"
          ...
          sub={
            health
              ? `${health.done_tasks}/${health.total_tasks} tasks`
              : '—'
          }
        />
        <Mini
          ...
          sub={estimated ? `de ${Math.round(estimated)}h estimadas` : 'sem estimativa'}
          accent={hoursOverflow ? 'text-danger' : 'text-text'}
        />
        <Mini
          label="Margem"
          value={margin != null ? `${Math.round(margin)}%` : '—'}
          sub={project.budget_amount ? formatBRL(Number(project.budget_amount)) : 'sem budget'}
          accent={
            margin == null
              ? 'text-text'
              : margin < 25
