...
export function TaskCard({ task, isDragging, onDragStart, onDragEnd }: TaskCardProps) {
  ...
  const code = project ? `${project.code}-${task.sequence_number}` : `#${task.sequence_number}`
  const priority = task.priority ? PRIORITY_CHIP[task.priority] : null
  const assigneeName = task.assignee?.profile?.full_name ?? null
  const isOverdue =
    task.due_date && new Date(task.due_date).getTime() < Date.now() && !task.completed_at
  ...
  return (
    ...
    >
      ...
      <h4 className="text-[13px] font-semibold leading-snug mb-2 line-clamp-2">{task.title}</h4>
      ...
      <div className="flex items-center justify-between text-[11px] gap-2">
        {assigneeName ? (
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-muted truncate">{assigneeName.split(' ')[0]}</span>
          </div>
        ) : (
      ...
      {task.estimated_hours != null && (
        <div className="mt-2 text-[10px] text-muted">
          {Number(task.actual_hours ?? 0).toFixed(0)}h / {Number(task.estimated_hours).toFixed(0)}h
        </div>
      )}
    </div>
  )
