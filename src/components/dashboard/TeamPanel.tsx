...
function loadLabel(
  ...
  if (percent < 30)
    return { text: 'Capacidade livre', chip: 'chip-status-todo' }
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted">
            Carga atual · ordenado por ocupação
          </p>
        </div>
      </div>
          ...
          <tbody>
            {members.map((m) => {
              ...
              return (
                <tr
                  key={m.member_id}
                  className="hover:bg-panel2/40 border-b last:border-0"
                >
                  ...
                  <td>
                    <span className="text-[12px]">
                      {m.open_tasks_count} ativa
                      {m.open_tasks_count !== 1 ? 's' : ''}
                    </span>
                  </td>
                  <td className="pr-1">
